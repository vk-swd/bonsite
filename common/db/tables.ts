...
type SqlType<T, NodeT> = { name: string, type: () => T, defaultParse: (s: string) => NodeT };
function makeSqlType<T, NodeT>(name: string, type: () => T, defaultParse: (s: string) => any): SqlType<T, NodeT> {
    return { name, type, defaultParse };
}

export const sqlTypes = {
    bigInt:         makeSqlType<any, number>('BIGINT',        () => sql.BigInt,            (s: string) => Number.parseInt(s)),
    dateTime:       makeSqlType<any, number>('DATETIME2(3)',  () => sql.DateTime2(3),      (s: string) => new Date(s).getTime()),
    decimal:        makeSqlType<any, number>('DECIMAL(18,2)', () => sql.Decimal(18, 2),    (s: string) => Number.parseFloat(s)),
    nvarchar:       makeSqlType<any, string>('NVARCHAR(100)', () => sql.NVarChar(100),     (s: string) => s),
    nvarcharSmall:  makeSqlType<any, string>('NVARCHAR(18)',  () => sql.NVarChar(18),      (s: string) => s),
    nvarcharBig:    makeSqlType<any, string>('NVARCHAR(max)', () => sql.NVarChar(sql.MAX), (s: string) => s),
    int:            makeSqlType<any, number>('INT',           () => sql.Int,               (s: string) => Number.parseInt(s)),
    tinyint:        makeSqlType<any, number>('TINYINT',       () => sql.TinyInt,           (s: string) => Number.parseInt(s))
}
...
export function makeCol<T, K extends keyof T, S>(
        nameA: K, 
        type: SqlType<S, T[K]>, 
        extra?: string, 
        value: Column<T,K>["value"] = (c:T) => Object(c)[nameA], 
        parse: Column<T,K>["parse"] = type.defaultParse): Column<T, K> {
...
export function parseQueryRes<T>(data: Stringed<T>, validator: Columns<T>): T {
    const parsed: any = {};
...
export const statTable: TableDescription<IdentityColumnT & { name: string, 
    value: number, reads: number, writes: number, busy: number, dataUnused: number, logUnused: number}> = {
    name: `${schema}.stats`,
    columns: {
        idx: makeCol('idx', sqlTypes.bigInt, 'identity(1,1) primary key'),
        name: makeCol('name', sqlTypes.nvarchar),
        value: makeCol('value', sqlTypes.bigInt),
        reads: makeCol('reads', sqlTypes.bigInt),
        writes: makeCol('writes', sqlTypes.bigInt),
        busy: makeCol('busy', sqlTypes.bigInt),
        logUnused: makeCol('logUnused', sqlTypes.bigInt),
        dataUnused: makeCol('dataUnused', sqlTypes.bigInt)
    },
    permissions: [
        { role: sinkRole, permissions: ['SELECT', 'INSERT', 'UPDATE'] }
    ]
}
export const statTableReads: TableDescription<IdentityColumnT & { name: string, 
    value: number}> = {
    name: `${schema}.statsRead`,
    columns: {
        idx: makeCol('idx', sqlTypes.bigInt, 'identity(1,1) primary key'),
        name: makeCol('name', sqlTypes.nvarchar),
        value: makeCol('value', sqlTypes.bigInt)
    ...
    permissions: [
        { role: statementCreatorRole, permissions: ['SELECT', 'INSERT', 'UPDATE'] }
    ]
}
...
export const transactionsTable: TableDescription<TransactionStored> = {
    ...
    columns: {
        idx: makeCol("idx", IdentityColumn.idx.type, IdentityColumn.idx.extra),
        id: makeCol("id", sqlTypes.bigInt, 'UNIQUE'),
        ...
        dateTime: makeCol("dateTime", sqlTypes.dateTime, undefined, (c) => new Date(c.dateTime).toISOString()),
...
export const transactionResultsTable: TableDescription<TransactionResultStored> ={
    ...
    columns: {
        idx: makeCol("idx", IdentityColumn.idx.type, IdentityColumn.idx.extra),
        id: makeCol('id', transactionsTable.columns.id.type, 'UNIQUE'),
        dateTime: makeCol('dateTime', sqlTypes.dateTime, 'NOT NULL', (c) => new Date(c.dateTime).toISOString()),
...
export const kafkaOffsetTable: TableDescription<Offset> = {
    ...
    columns: {
        ...
        offset: makeCol('offset', sqlTypes.nvarchar, 'NOT NULL')
    }
    , permissions: [
        { role: sinkRole, permissions: ['SELECT', 'INSERT', 'UPDATE'] }
    ]
...
type RawData = IdentityColumnT & { data: string};
